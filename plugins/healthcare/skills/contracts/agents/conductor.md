...
2. **Scope** — turn the brief's scope intent into a concrete read set. Filter on `documents` provenance columns (publisher/category/dated/family — these are hard facts), grep `documents.content` for the brief's vocabulary + knowledge-index synonyms, then read the `summary` column of candidates to rank. Write a `scopes` row + `scope_documents` rows. `predicate` is what you actually applied; `terms` is the vocabulary you learned for this question — entity aliases, d/b/a names, acronyms, domain phrases — recorded so a reviewer can see what you knew even when the predicate only needed one headword. Aggregates / negatives / "which contracts lack X" → no cap, full sweep. Large exclusions go to the queue **before** sweeping.
