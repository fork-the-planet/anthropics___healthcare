...
- **Whenever the user gives you a location** — typed path, picked from a list, or a folder dragged into chat — **acknowledge it in words before any tool call**: "Got it — I can see your contracts folder. Taking a look now." A reply that opens with a Bash call and no text looks like a blank message in the desktop app.
...
command -v bun >/dev/null || { echo "SETUP_ERROR: could not set up runtime"; exit 0; }

ROOT="${CLAUDE_SKILL_DIR}"
CLI="$ROOT/scripts/cli.ts"
[ -d "$ROOT/node_modules" ] || (cd "$ROOT" && bun install --silent)
DATA="${CLAUDE_HEALTHCARE_DATA:-$HOME/.claude/data/healthcare}/contracts"
mkdir -p "$DATA/reports"

BUN="$(command -v bun)"
echo "BUN=$BUN"; echo "CLI=$CLI"; echo "ROOT=$ROOT"; echo "DATA=$DATA"
echo "--- corpora candidates ---"
find /mnt . -maxdepth 7 -type d -name corpora 2>/dev/null | grep -v node_modules
```

The block above auto-installs bun if it isn't already present (and picks up `~/.bun/bin` even when it's not on the default PATH), so a fresh machine just works — **never ask the user to install bun**. If `SETUP_ERROR` printed, the auto-install failed (almost always a network problem). Tell the user: "I couldn't finish getting ready — please check your internet connection and try `/contracts` again." Don't mention bun or the script.
...
**For the corpus**, list the candidates the bootstrap printed and ask the user to confirm which one — **always confirm, even if only one was found**, so they know what set the answer will be drawn from. They may pick from the list, type a path, or drag a folder into chat — all are fine. **When they answer, your reply must open with a plain-English acknowledgement** ("Got it — I can see your contracts at `<path>`. Let me check what's there.") **before** any Bash call, so they see confirmation rather than a blank message. The expected layout is `corpora/<name>/*.{pdf,docx,xlsx,pptx,txt,md,html}` (one file per document). PDF/DOCX/XLSX/PPTX are converted to page-anchored text automatically during `ingest`.

Once confirmed, you'll use these literal paths everywhere below: `<BUN>`, `<CLI>`, `<ROOT>`, `<DATA>`, `<CORPORA>` (the path the user picked).
...
2. **Create the run and spawn the engine.** Write the run row:
   ...
   >
   > ---
   > RUN_ID=`<RUN_ID>`  BUN=`<BUN>`  CLI=`<CLI>`  ROOT=`<ROOT>`  CORPORA=`<CORPORA>`  corpus=`<name>`
   >
   > <user_question>
